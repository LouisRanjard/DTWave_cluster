...
[wintime, hoptime, numcep, lifterexp, sumpower, preemph, dither, ...
 minfreq, maxfreq, nbands, bwidth, dcttype, fbtype, usecmp, modelorder, broaden] = ...
    process_options(varargin, 'wintime', 0.025, 'hoptime', 0.010, ...
	  ...
	  'fbtype', 'mel', 'usecmp', 0, 'modelorder', 0, 'broaden', 0);
...
pspectrum = powspec(samples, sr, wintime, hoptime, dither, preemph);
...
% 0th cepstra is at the end
cepstra = [cepstra(2:end,:);cepstra(1,:)];

% mean removal
cepstra = cepstra - repmat(mean(cepstra,2),1,size(cepstra,2));

% add deltas
cepstra = [cepstra; deltas(cepstra,5); deltas(deltas(cepstra,5), 5)];
